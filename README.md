...
Stan implementation of the single-season SPL model, used as a NUTS baseline against the custom Python samplers.
...
- **Jacobian correction for flat/lognormal_gamma**: `target += log_phi + log_eta` is added explicitly when `prior_type != 1`, matching the change-of-variables term in `posteriors.py` (which does not cancel against the prior for these types)
...
Runs the custom Python samplers (RWMH and MALA) across every combination of truth model, inference model, and prior type for the single-season base model.

**Key Components:**
- **27-combination outer loop**: 3 truth models x 3 recover models x 3 prior types; data is generated once per truth model and reused across all recover model x prior type pairs, enabling fair model comparison on the same dataset
...
- **Outputs to `Stats/`**: posterior summary CSVs (`posterior_summary_base_{truth}_{recover}_{prior}_{sampler}.csv`) and per-parameter ESS, ESS/s, and R-hat diagnostics tables; skips existing files so runs can be interrupted and resumed
...
**What to look at:**
- **18-combination outer loop**: 3 truth models x 3 prior types x 2 parameterizations (centered, noncentered); only uses SPL recover model; tests both parameterization to expose the funnel geometry problem under centered coordinates
- **Same MALA preconditioning strategy** as the base runner
- **Outputs to `Stats/`**: posterior summary and diagnostics CSVs with `_hierarchical_` prefix, in the same format as the base runner for consistency
